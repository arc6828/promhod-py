# การปรับปรุงระบบสกัด รวบรวนข้อมูล และสรุปเหตุการน้ำท่วนข้อวออนไลน์ด้วยมูเดลภาาขนาดใหญ์
...
> *// 1. ความสำคัญของข้อมูลเหตุการณ์น้ำท่วมจากสื่อออนไลน์*

ในยุคข้อมูลขนาดใหญ่ (Big Data) สื่อออนไลน์และข่าวสารดิจิทัลได้กลายเป็นแหล่งข้อมูลทางเลือกที่มีความสำคัญอย่างยิ่งในการติดตามสถานการณ์ภัยพิบัติได้อย่างรวดเร็วและครอบคลุม (Eze et al., 2020) เนื่องจากเหตุการณ์น้ำท่วมมักเกิดขึ้นอย่างฉับพลันและส่งผลกระทบเป็นบริเวณกว้าง การรายงานข่าวเชิงพื้นที่จากผู้สื่อข่าวภาคสนามหรือการรายงานข่าวท้องถิ่นออนไลน์จึงสามารถสะท้อนข้อมูลจริงได้เร็วกว่ารายงานอย่างเป็นทางการของหน่วยงานภาครัฐ ซึ่งบ่อยครั้งมีกระบวนการรายงานตามลำดับขั้นที่ซับซ้อน ส่งผลให้เกิดความล่าช้าของข้อมูล (Information Latency) หรือขาดรายละเอียดเชิงลึกในบางพื้นที่ห่างไกลที่ระบบตรวจวัดทางกายภาพ (เช่น สถานีวัดระดับน้ำ) ยังเข้าไม่ถึง การนำข้อมูลเหล่านี้มาประมวลผลและวิเคราะห์เชิงพื้นที่และเวลา (Spatiotemporal Analysis) จึงเป็นแนวทางเสริมประสิทธิภาพการรายงานของภาครัฐ ช่วยลดความล่าช้าในการรวบรวมข้อมูล และเพิ่มความครอบคลุมเพื่อสร้างระบบเตือนภัยล่วงหน้าที่ตอบสนองต่อเหตุการณ์ได้ทันท่วงที (Revilla-Romero et al., 2015)
...
จากการทบทวนวรรณกรรมข้างต้น ผู้วิจัยได้ทำการสรุปเปรียบเทียบจุดแข็งและข้อจำกัดของเทคโนโลยีแต่ละประเภทที่นิยมนำมาใช้ในการสกัดข้อมูลภัยพิบัติเชิงเหตุการณ์ออนไลน์ ได้แก่ Traditional NER, BERT-based NER และ Large Language Models (LLMs) ดังรายละเอียดแสดงในตารางที่ 2
...
| **Large Language Models**<br>(เช่น Gemini API) | ใช้โมเดลภาษาขนาดใหญ่ที่ผ่านการฝึกฝนเชิงสร้างสรรค์ (Generative model) วิเคราะห์คำสั่ง (Prompt) เพื่อตีความและสกัดข้อมูลจากข้อความภาษาธรรมชาติโดยตรง | มีความยืดหยุ่นสูง รองรับการสกัดข้อมูลแบบ Zero-shot โดยไม่ต้องปรับแต่งค่าน้ำหนักโมเดลใหม่ (Fine-tuning) สามารถสกัดมิติบริบท ระดับความรุนแรง และสรุปเนื้อหาข่าวได้พร้อมกัน | มีค่าใช้จ่ายตามปริมาณการใช้งาน (Token-based pricing) จำเป็นต้องเชื่อมต่อเครือข่ายอินเทอร์เน็ตกับ API ภายนอก และต้องคำนึงถึงประเด็นความปลอดภัยและความเป็นส่วนตัวของข้อมูล |

## 3. สถาปัตยกรรมระบบและการระเบียบวิธีวิจัย (Proposed System Architecture and Methodology)

> [!NOTE]
> 💬 **[ข้อคิดเห็นจากผู้ทรงคุณวุฒิ - สถาปัตยกรรมและกระบวนการทำงาน]** — 🟩 **[แก้ไขแล้ว (Resolved)]**:
> ~~1. ควรเพิ่มแผนภาพแสดงโครงสร้างและสถาปัตยกรรมระบบที่เสนอ (System Architecture Diagram) เพื่อให้เห็นทิศทางการไหลของข้อมูล (Data Flow) ตั้งแต่ Web Scraping, n8n Workflow, Gemini API ไปจนถึง Visualization~~
...
### 3.5 ข้อมูลสเปกเครื่องประมวลผลและสภาพแวดล้อมที่ใช้จริง

เพื่อความเที่ยงตรงและยึดถือหลักการทดลองซ้ำได้ (Reproducibility) การทดสอบประสิทธิภาพของทั้งสองระบบดำเนินการภายใต้สภาพแวดล้อมฮาร์ดแวร์และซอฟต์แวร์บนเครื่องประมวลผลเดียวกัน (DESKTOP-K4BH547: HP ProDesk 400 G3 DM) ซึ่งประกอบด้วยหน่วยประมวลผลกลาง Intel Core i5-6500T CPU @ 2.50 GHz, หน่วยความจำหลัก 12.0 GB DDR4 (2,133 MHz), หน่วยจัดเก็บข้อมูล 932 GB SSD (WD Blue SA510 2.5 1000GB) และระบบปฏิบัติการ Microsoft Windows 10 Pro 64-bit โดยเครื่องทดสอบใช้หน่วยประมวลผลกราฟิกแบบออนบอร์ด Intel HD Graphics 530 (128 MB) โดยไม่มีการติดตั้งการ์ดแสดงผลแยก (Discrete GPU) ส่งผลให้ระบบเดิมต้องรันแบบจำลอง WangchanBERTa (`airesearch/wangchanberta-base-att-spm-uncased`) ผ่านรันไทม์ Python 3.10+ ในโหมดคำนวณด้วยซีพียูทั้งหมด (CPU-only Inference) ในขณะที่ระบบใหม่ที่นำเสนอทำงานบนรันไทม์ Node.js v22.21.1 ร่วมกับเครื่องมือควบคุมลำดับงาน n8n Workflow Automation Engine เพื่อส่งคำร้องสกัดข้อมูลไปยัง Google Gemini API ผ่านเครือข่ายความเร็วสูง ช่วยก้าวข้ามข้อจำกัดด้านฮาร์ดแวร์ของเครื่องประมวลผลส่วนบุคคลได้อย่างมีประสิทธิภาพ
...
### 5.4. การวิเคราะห์เชิงตัวเลขด้านต้นทุนและปริมาณโทเคน (Token and Cost Analysis)

เนื่องจากต้นทุนและประสิทธิภาพการประมวลผลด้วยโมเดลภาษาขนาดใหญ่ (LLM) แปรผันโดยตรงตามจำนวนโทเคน (Tokens) ทั้งในฝั่งขาเข้า (Input Tokens) และขาออก (Output Tokens) การวิเคราะห์ในส่วนนี้จึงสรุปเชิงปริมาณครอบคลุม **2 ประเด็นหลัก** ดังรายละเอียดต่อไปนี้:

#### 1. ประเด็นปริมาณโทเคนที่ใช้ในการประมวลผล (Token Consumption Analysis)
จากการทดสอบประมวลผลสกัดข้อมูลและสรุปเนื้อหาจากชุดข้อมูลข่าวอุทกภัยทั้งหมดจำนวน 554 บทความ ในโฟลเดอร์ [`news/data-detail2`](file:///d:/python/promhod-env/promhod-py/news/data-detail2) โดยใช้แบบจำลองการตัดคำ (SentencePiece Subword Tokenizer) ของ Gemini พบว่าระบบใช้ปริมาณโทเคนรวมทั้งสิ้น 900,842 Tokens (~0.90 ล้านโทเคน) โดยแบ่งเป็นปริมาณ Input Tokens จำนวน 872,318 Tokens (คิดเป็นสัดส่วนสูงถึงร้อยละ 96.84) และ Output Tokens จำนวน 28,524 Tokens (คิดเป็นร้อยละ 3.16) ซึ่งแสดงให้เห็นว่าภาระการประมวลผลส่วนใหญ่อยู่ในฝั่งข้อมูลขาเข้า ขณะที่ฝั่งข้อมูลขาออกใช้โทเคนเพียงส่วนน้อยเนื่องจากสถาปัตยกรรมคำสั่งกำหนดให้โมเดลตอบกลับเฉพาะโครงสร้างข้อมูล JSON ที่มีความกระชับสูง

#### 2. ประเด็นราคาและค่าใช้จ่ายที่ต้องจ่าย (Cost and Pricing Analysis)
จากการตรวจสอบอัตราค่าบริการมาตรฐานของการใช้งาน Gemini API สำหรับโมเดล Gemini 2.5 Flash (Google Developers, 2024; กำหนดอัตราบริการ Input $0.300 USD/1M Tokens และ Output $2.500 USD/1M Tokens อัตราแลกเปลี่ยนอ้างอิง 1 USD = 36.00 THB) คิดเป็นค่าบริการฝั่ง Input Tokens สำหรับชุดข้อมูลข่าวทั้งสิ้น 554 ข่าว (872,318 Tokens) เท่ากับ $0.2617 USD (~9.42 บาทไทย) และค่าบริการฝั่ง Output Tokens (28,524 Tokens) เท่ากับ $0.0713 USD (~2.57 บาทไทย) ส่งผลให้ต้นทุนค่าใช้จ่ายรวมทั้งสิ้นสำหรับการสกัดและสรุปข่าวน้ำท่วมตลอดทั้งชุดข้อมูล คิดเป็นเงินเพียง $0.3330 USD หรือประมาณ 11.99 บาทไทย เท่านั้น (ตกเฉลี่ยเพียงประมาณ 0.0216 บาทต่อ 1 บทความข่าว)

ในด้านความเป็นไปได้เชิงเศรษฐศาสตร์และการนำไปประยุกต์ใช้งานจริง หากออกแบบระบบรับและประมวลผลข่าวสารกึ่งเรียลไทม์ (Streaming Ingestion) ระบบจะสามารถทำงานภายใต้โควตาใช้งานฟรี (Free Tier) ของ Google AI Studio (รองรับสูงสุด 15 Requests/Minute และ 1,500 Requests/Day) ได้โดยไม่มีค่าใช้จ่าย (0 บาท; Google Developers, 2024) นอกจากนี้ เมื่อเปรียบเทียบกับระบบ NER เดิมบน Python ที่ต้องมีภาระต้นทุนในการจัดหาและบำรุงรักษาเครื่องเซิร์ฟเวอร์พร้อมการ์ดประมวลผลกราฟิก (GPU) แบบ On-Premise ตลอด 24 ชั่วโมง สถาปัตยกรรมใหม่ที่ใช้ JavaScript (Node.js) ร่วมกับ n8n Workflow และ Gemini API จึงสามารถลดต้นทุนการดำเนินงาน (OpEx) ลงได้อย่างมหาศาล และมีความคุ้มค่าสูงยิ่งสำหรับหน่วยงานภาครัฐหรือองค์กรปกครองส่วนท้องถิ่นที่มีข้อจำกัดด้านงบประมาณ
...
Google Developers. (2024). Google AI Studio Pricing and Model Documentation. Google AI for Developers. Retrieved from https://ai.google.dev/pricing
